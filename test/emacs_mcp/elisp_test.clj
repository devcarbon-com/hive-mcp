(ns emacs-mcp.elisp-test
  "Comprehensive unit tests for the emacs-mcp.elisp namespace.
...
(deftest test-require-and-call-json-no-args
  (testing "Basic usage without arguments"
    (let [result (el/require-and-call-json 'emacs-mcp-magit 'emacs-mcp-magit-api-status)]
      ;; Should contain progn wrapper
      (is (str/starts-with? result "(progn"))
      ;; Should require the feature
      (is (str/includes? result "(require 'emacs-mcp-magit nil t)"))
      ;; Should check if function is bound
      (is (str/includes? result "(fboundp 'emacs-mcp-magit-api-status)"))
      ;; Should call the function
      (is (str/includes? result "(emacs-mcp-magit-api-status)"))
      ;; Should JSON-encode the result
      (is (str/includes? result "(json-encode (emacs-mcp-magit-api-status))"))
      ;; Should have error handling
      (is (str/includes? result "emacs-mcp-magit not loaded")))))

(deftest test-require-and-call-json-single-arg
  (testing "Usage with single numeric argument"
    (let [result (el/require-and-call-json 'emacs-mcp-magit 'emacs-mcp-magit-api-log 10)]
      ;; Should include the argument
      (is (str/includes? result "(emacs-mcp-magit-api-log 10)"))
      (is (str/includes? result "(json-encode (emacs-mcp-magit-api-log 10))"))))

  (testing "Usage with single string argument"
    (let [result (el/require-and-call-json 'emacs-mcp-cider 'emacs-mcp-cider-eval "(+ 1 2)")]
      ;; String should be properly quoted
      (is (str/includes? result "(emacs-mcp-cider-eval \"(+ 1 2)\")"))))

  (testing "Usage with single symbol argument (should be quoted)"
    (let [result (el/require-and-call-json 'emacs-mcp-magit 'emacs-mcp-magit-api-diff 'staged)]
      ;; Symbol should be quoted with '
      (is (str/includes? result "(emacs-mcp-magit-api-diff 'staged)")))))

(deftest test-require-and-call-json-multiple-args
  (testing "Usage with multiple arguments of different types"
    (let [result (el/require-and-call-json 'emacs-mcp-test 'test-fn "hello" 42 'world)]
...
(deftest test-require-and-call-json-nil-arg
  (testing "Usage with nil argument"
    (let [result (el/require-and-call-json 'emacs-mcp-test 'test-fn nil)]
      (is (str/includes? result "(test-fn nil)")))))

(deftest test-require-and-call-json-keyword-arg
  (testing "Usage with keyword argument"
    (let [result (el/require-and-call-json 'emacs-mcp-test 'test-fn :silent)]
...
(deftest test-require-and-call-json-vector-arg
  (testing "Usage with vector argument (becomes quoted list)"
    (let [result (el/require-and-call-json 'emacs-mcp-test 'test-fn ["a" "b"])]
...
(deftest test-require-and-call-text-no-args
  (testing "Basic usage without arguments"
    (let [result (el/require-and-call-text 'emacs-mcp-magit 'emacs-mcp-magit-api-diff)]
      ;; Should contain progn wrapper
      (is (str/starts-with? result "(progn"))
      ;; Should require the feature
      (is (str/includes? result "(require 'emacs-mcp-magit nil t)"))
      ...
      (is (str/includes? result "\"Error: emacs-mcp-magit not loaded\"")))))

(deftest test-require-and-call-text-with-args
  (testing "Usage with symbol argument"
    (let [result (el/require-and-call-text 'emacs-mcp-magit 'emacs-mcp-magit-api-diff 'staged)]
      (is (str/includes? result "(emacs-mcp-magit-api-diff 'staged)")))))

(deftest test-require-and-call-text-multiple-args
  (testing "Usage with multiple arguments"
    (let [result (el/require-and-call-text 'emacs-mcp-test 'test-fn "file.txt" 100 'mode)]
...
(deftest test-require-and-call-basic
  (testing "Basic usage without arguments"
    (let [result (el/require-and-call 'emacs-mcp-magit 'emacs-mcp-magit-api-stage)]
      ;; Should contain progn wrapper
      (is (str/starts-with? result "(progn"))
      ;; Should require the feature
      (is (str/includes? result "(require 'emacs-mcp-magit nil t)"))
      ;; Should check fboundp
      (is (str/includes? result "(fboundp 'emacs-mcp-magit-api-stage)"))
      ;; Should call the function
      (is (str/includes? result "(emacs-mcp-magit-api-stage)"))
...
(deftest test-require-and-call-with-args
  (testing "Usage with arguments"
    (let [result (el/require-and-call 'emacs-mcp-magit 'emacs-mcp-magit-api-stage ["file1.txt" "file2.txt"])]
      (is (str/includes? result "(emacs-mcp-magit-api-stage '(\"file1.txt\" \"file2.txt\"))")))))
...
(deftest test-fboundp-call-json-no-args
  (testing "Basic usage without arguments"
    (let [result (el/fboundp-call-json 'emacs-mcp-api-status)]
      ;; Should check fboundp first
      (is (str/starts-with? result "(if (fboundp 'emacs-mcp-api-status)"))
      ;; Should JSON-encode result
      (is (str/includes? result "(json-encode (emacs-mcp-api-status))"))
      ;; Should have error case
      (is (str/includes? result "emacs-mcp-api-status not available")))))

(deftest test-fboundp-call-json-with-args
  (testing "Usage with arguments"
    (let [result (el/fboundp-call-json 'emacs-mcp-api-get "buffer-name" 42)]
      (is (str/includes? result "(emacs-mcp-api-get \"buffer-name\" 42)")))))

(deftest test-fboundp-call-json-symbol-arg
  (testing "Usage with symbol argument"
    (let [result (el/fboundp-call-json 'emacs-mcp-get-mode 'text-mode)]
      (is (str/includes? result "(emacs-mcp-get-mode 'text-mode)")))))
...
(deftest test-real-world-magit-usage
  (testing "Real-world magit status call"
    (let [result (el/require-and-call-json 'emacs-mcp-magit 'emacs-mcp-magit-api-status)]
      (is (str/starts-with? result "(progn"))
      (is (str/includes? result "require 'emacs-mcp-magit"))
...
(deftest test-real-world-cider-eval
  (testing "Real-world cider eval call with code string"
    (let [result (el/require-and-call-json 'emacs-mcp-cider 'emacs-mcp-cider-eval "(+ 1 2 3)")]
      (is (str/includes? result "(emacs-mcp-cider-eval \"(+ 1 2 3)\")")))))
