...
The addon system provides modular integrations between hive-mcp and other Emacs packages.
...
- *Consistent API*: All addons register via =hive-mcp-addon-register=
...
Integrates [[https://github.com/karthink/claude-code][claude-code.el]] (Claude Code CLI interface) with hive-mcp.
...
#+begin_src elisp
(hive-mcp-addon-load 'claude-code)
...
Integrates [[https://github.com/clojure-emacs/cider][CIDER]] (Clojure IDE) with hive-mcp.
...
(setq hive-mcp-cider-auto-log-results t)
(setq hive-mcp-cider-log-threshold 100)
#+end_src
...
Integrates [[https://github.com/rksm/org-ai][org-ai]] with hive-mcp.
...
(setq hive-mcp-org-ai-auto-context t)

;; Auto-save conversations
(setq hive-mcp-org-ai-auto-save t)

;; Context sections to include
(setq hive-mcp-org-ai-context-sections '(buffer project git))
#+end_src
...
┌─────────────────────────────────────────────────────────┐
│                   Unified API                           │
│  hive-mcp-kanban-{list,create,update,move}-task       │
├─────────────────────┬───────────────────────────────────┤
...
(setq hive-mcp-kanban-backend 'standalone)

;; For vibe-kanban cloud sync
(setq hive-mcp-kanban-backend 'vibe)
(setq hive-mcp-kanban-default-project "your-project-uuid")

;; Enable dual-backend with auto-sync
(setq hive-mcp-kanban-enable-dual-backend t)
...
(setq hive-mcp-kanban-track-agents t)
#+end_src
...
#+begin_src bash
cp elisp/addons/hive-mcp-addon-template.el \
   elisp/addons/hive-mcp-my-addon.el
#+end_src

** Structure

#+begin_src elisp
;;; hive-mcp-my-addon.el --- My integration -*- lexical-binding: t; -*-
...
(defun hive-mcp-my-addon-do-thing ()
...
;;; Minor mode
(define-minor-mode hive-mcp-my-addon-mode
  "My addon mode."
  :lighter " MCP-My"
  :global t)
...
(with-eval-after-load 'hive-mcp-addons
  (hive-mcp-addon-register
   ...
   :requires '(target-package hive-mcp-api)
   :provides '(hive-mcp-my-addon-mode)))
...
#+begin_src elisp
;; In init.el after loading hive-mcp
