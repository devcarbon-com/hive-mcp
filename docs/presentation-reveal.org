...
** Architecture
#+ATTR_REVEAL: :frag (appear)
- *Clojure MCP Server* — STDIO communication
- *Elisp Package* — hive-mcp.el
- *org-clj Library* — Native org-mode parser
...
- 🏗️ *decision* — Architecture choices
...
(hive-mcp-swarm-spawn "reviewer" :role 'reviewer)

;; Broadcast to all
(hive-mcp-swarm-broadcast "Report status")
#+end_src
...
#+ATTR_REVEAL: :frag (appear)
- *Overarch* — C4 diagrams (context, container, component)
- *Mermaid* — Flowcharts, sequence diagrams
- *TikZ* — LaTeX-quality vector graphics

*** C4 Context Example
#+ATTR_HTML: :width 600px
[[file:images/hive-mcp-c4-context.png]]

*** Code That Generated It
#+begin_src clojure
{:type :c4-context
 :title "hive-mcp System Context"
 :elements [{:id :developer :name "Developer" :el :person}
            {:id :claude :name "Claude AI" :el :system :external? true}
            {:id :hive-mcp :name "hive-mcp" :el :system}
            {:id :emacs :name "Emacs" :el :system}]
 :relations [{:from :developer :to :emacs :name "edits code"}
             {:from :claude :to :hive-mcp :name "MCP protocol"}
             {:from :hive-mcp :to :emacs :name "emacsclient"}]}
#+end_src
...
- Claude: Creates =hive-mcp-presentation.el=
- User: "Update my email"
- Claude: Edits file → exports → opens browser
- Instant feedback loop!

** The Interaction Flow
#+ATTR_REVEAL: :frag (appear)
1. *User Request* → Natural language prompt
2. *Claude Processing* → Selects appropriate MCP tools
3. *Tool Execution* → =read_file=, =file_edit=, =eval_elisp=
4. *Emacs Actions* → Buffer ops, export, browse-url
5. *Result* → Browser shows updated presentation

** Capabilities Used
| Tool | Purpose |
|------|---------|
| =mcp__clojure-mcp-emacs__file_edit= | Edit org files |
| =mcp__hive-mcp__eval_elisp= | Export & preview |
| =mcp__hive-mcp__mcp_memory_add= | Save conventions |
| =mcp__dynamic-kanban__add_feature= | Track tasks |

** Why This Matters
#+ATTR_REVEAL: :frag (appear)
- *Zero context switching* — Stay in Emacs
- *Persistent memory* — Claude remembers conventions
- *Composable tools* — Build complex workflows
- *Immediate feedback* — See results instantly

* Questions?
** Links
#+ATTR_REVEAL: :frag (appear)
- 🔗 GitHub: [[https://github.com/BuddhiLW][github.com/BuddhiLW]]
- 💼 LinkedIn: [[https://www.linkedin.com/in/pedro-g-branquinho/][pedro-g-branquinho]]
