...
Diagram-building capabilities for hive-mcp ecosystem using an adapter-based
...
Located in: =src/hive_mcp/diagrams/core.clj=
...
File: =src/hive_mcp/diagrams/adapters/overarch.clj=
...
** TikZ Adapter (LaTeX)
File: =src/hive_mcp/diagrams/adapters/tikz.clj=
...
** Mermaid Adapter (Web)
File: =src/hive_mcp/diagrams/adapters/mermaid.clj=
...
File: =src/hive_mcp/diagrams/tools.clj=
...
| =diagram_hive_mcp_architecture= | Generate system architecture |

* Architecture Diagram

#+begin_src 
┌─────────────────────────────────────────────────────────────┐
│                    hive-mcp Diagram System                  │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌──────────────┐    ┌───────────────────────────────────┐  │
...
** TODO hive-mcp-diagrams.el addon
Create Emacs interface for the Clojure diagram system:

*** Commands
| Command | Description |
|---------+-------------|
| =hive-mcp-diagram-create= | Interactive diagram creation |
| =hive-mcp-diagram-compile= | Compile to output format |
| =hive-mcp-diagram-preview= | Preview in buffer/browser |
| =hive-mcp-diagram-from-region= | Create from selected EDN |

*** Transient Menu
#+begin_src elisp
(transient-define-prefix hive-mcp-diagram-menu ()
  "Diagram creation menu."
  ["Create"
   ("f" "Flowchart" hive-mcp-diagram-flowchart)
   ("s" "Sequence" hive-mcp-diagram-sequence)
   ("c" "C4 Context" hive-mcp-diagram-c4-context)
   ("a" "Architecture" hive-mcp-diagram-architecture)]
  ["Output"
   ("h" "HTML" hive-mcp-diagram-to-html)
   ("p" "PDF" hive-mcp-diagram-to-pdf)
   ("t" "TikZ" hive-mcp-diagram-to-tikz)])
#+end_src
