;;; emacs-mcp-presentation.el --- Org-mode presentation addon for emacs-mcp -*- lexical-binding: t; -*-
...
;;
;; Usage:
;;   M-x emacs-mcp-presentation-create   ; Create new presentation
;;   M-x emacs-mcp-presentation-transient ; Open menu
...
(defgroup emacs-mcp-presentation nil
  "Org-mode presentation creation with emacs-mcp."
  :group 'emacs-mcp
  :prefix "emacs-mcp-presentation-")

(defcustom emacs-mcp-presentation-default-format 'beamer
  "Default presentation format."
  :type '(choice (const :tag "Beamer (LaTeX/PDF)" beamer)
                 (const :tag "Reveal.js (HTML)" revealjs))
  :group 'emacs-mcp-presentation)

(defcustom emacs-mcp-presentation-author user-full-name
  "Default author name for presentations."
  :type 'string
  :group 'emacs-mcp-presentation)

(defcustom emacs-mcp-presentation-email user-mail-address
  "Default email for presentations."
  :type 'string
  :group 'emacs-mcp-presentation)

(defcustom emacs-mcp-presentation-beamer-theme "magpie"
  "Default Beamer color theme."
  :type 'string
  :group 'emacs-mcp-presentation)

(defcustom emacs-mcp-presentation-reveal-theme "blood"
  "Default Reveal.js theme."
  :type 'string
  :group 'emacs-mcp-presentation)

(defcustom emacs-mcp-presentation-templates-dir
...
(defconst emacs-mcp-presentation--beamer-header
...
(defconst emacs-mcp-presentation--revealjs-header
  ":REVEAL_PROPERTIES:
...
(defconst emacs-mcp-presentation--beamer-slide
  "*** %s
%s
"
  "Beamer slide template (level 3 heading).")

(defconst emacs-mcp-presentation--beamer-two-column
  "*** %s
**** %s
:PROPERTIES:
:BEAMER_COL: 0.48
...
(defconst emacs-mcp-presentation--revealjs-slide
  "** %s
%s
"
  "Reveal.js slide template (level 2 heading).")

(defconst emacs-mcp-presentation--revealjs-iframe
  "** %s
:PROPERTIES:
...
(defun emacs-mcp-presentation--generate-header (format title &optional subtitle)
  "Generate presentation header for FORMAT with TITLE and optional SUBTITLE."
  (pcase format
    ('beamer
     (format emacs-mcp-presentation--beamer-header
             title
             emacs-mcp-presentation-email
             (car (split-string emacs-mcp-presentation-author))
             emacs-mcp-presentation-author
             emacs-mcp-presentation-email
             (or subtitle "")
             emacs-mcp-presentation-beamer-theme))
    ('revealjs
     (format emacs-mcp-presentation--revealjs-header
             emacs-mcp-presentation-reveal-theme
             title
             emacs-mcp-presentation-author))
...
;;;###autoload
(defun emacs-mcp-presentation-create (title format filename)
  ...
  (interactive
   (list (read-string "Presentation title: ")
         (intern (completing-read "Format: " '("beamer" "revealjs") nil t
                                  (symbol-name emacs-mcp-presentation-default-format)))
         (read-file-name "Save as: " nil nil nil "presentation.org")))
  (let ((header (emacs-mcp-presentation--generate-header format title)))
    (find-file filename)
    (insert header)
    (insert (format "* %s\n" title))
    (pcase format
...
;;;###autoload
(defun emacs-mcp-presentation-insert-slide (title &optional content)
  "Insert a new slide with TITLE and optional CONTENT."
  (interactive "sSlide title: ")
  (let ((format (emacs-mcp-presentation--detect-format)))
    (end-of-line)
    (insert "\n")
    (insert (pcase format
              ('beamer (format emacs-mcp-presentation--beamer-slide
                               title (or content "")))
              ('revealjs (format emacs-mcp-presentation--revealjs-slide
                                 title (or content "")))
              (_ (format "** %s\n%s\n" title (or content "")))))))

;;;###autoload
(defun emacs-mcp-presentation-insert-two-column (title left-title left-content right-title right-content)
...
RIGHT-TITLE and RIGHT-CONTENT for right column."
  ...
  (let ((format (emacs-mcp-presentation--detect-format)))
    ...
    (pcase format
      ('beamer
       (insert (format emacs-mcp-presentation--beamer-two-column
...
;;;###autoload
(defun emacs-mcp-presentation-insert-code-slide (title language code)
...
;;;###autoload
(defun emacs-mcp-presentation-insert-image-slide (title image-path &optional caption)
...
;;;###autoload
(defun emacs-mcp-presentation-insert-quote-slide (title quote author)
...
;;;###autoload
(defun emacs-mcp-presentation-insert-iframe-slide (title url)
  ...
  (let ((format (emacs-mcp-presentation--detect-format)))
    ...
    (pcase format
      ('revealjs
       (insert (format emacs-mcp-presentation--revealjs-iframe title url)))
      (_
       (insert (format "** %s\nSee: %s\n" title url))
...
;;;###autoload
(defun emacs-mcp-presentation-export ()
...
;;;###autoload
(defun emacs-mcp-presentation-preview ()
...
;;;###autoload
(defun emacs-mcp-presentation-export-and-preview ()
...
For Reveal.js: exports to HTML and opens in browser."
  (interactive)
  (emacs-mcp-presentation-export)
  (emacs-mcp-presentation-preview))

;;;###autoload
(defun emacs-mcp-presentation-refresh ()
...
;;;###autoload
(defun emacs-mcp-presentation-open-file (file)
  "Open presentation FILE, export it, and preview.
FILE should be an org-mode presentation file."
  (interactive "fPresentation file: ")
  (find-file file)
  (emacs-mcp-presentation-export-and-preview))

;;;###autoload
(defun emacs-mcp-presentation-open-html-in-browser (file)
...
;;;###autoload
(defun emacs-mcp-presentation-open-current-html ()
  "Open the HTML version of current presentation in browser."
  (interactive)
  (emacs-mcp-presentation-open-html-in-browser buffer-file-name))

;;;; Helper Functions:

(defun emacs-mcp-presentation--detect-format ()
...
;;;###autoload
(defun emacs-mcp-presentation-generate-outline (topic)
  "Generate a presentation outline for TOPIC using MCP memory context."
  (interactive "sPresentation topic: ")
  (when (emacs-mcp-api-available-p)
    (let* ((conventions (emacs-mcp-api-memory-query "convention" nil 5))
           (notes (emacs-mcp-api-memory-query "note" nil 5))
...
;;;###autoload
(defun emacs-mcp-presentation-save-slide-to-memory ()
  "Save the current slide to MCP memory as a snippet."
  (interactive)
  (when (emacs-mcp-api-available-p)
    (let* ((slide-content (emacs-mcp-presentation--get-current-slide))
           (title (emacs-mcp-presentation--get-slide-title)))
      (emacs-mcp-api-memory-add
       "snippet"
       slide-content
...
(defun emacs-mcp-presentation--get-current-slide ()
...
(defun emacs-mcp-presentation--get-slide-title ()
...
(transient-define-prefix emacs-mcp-presentation-transient ()
  "Presentation creation menu."
  ["emacs-mcp Presentations"
   ["Create"
    ("n" "New presentation" emacs-mcp-presentation-create)
    ("s" "Insert slide" emacs-mcp-presentation-insert-slide)
    ("2" "Two-column slide" emacs-mcp-presentation-insert-two-column)]
   ["Content"
    ("c" "Code slide" emacs-mcp-presentation-insert-code-slide)
    ("i" "Image slide" emacs-mcp-presentation-insert-image-slide)
    ("q" "Quote slide" emacs-mcp-presentation-insert-quote-slide)
    ("f" "Iframe slide (Reveal)" emacs-mcp-presentation-insert-iframe-slide)]
   ["Export & Preview"
    ("e" "Export" emacs-mcp-presentation-export)
    ("p" "Preview" emacs-mcp-presentation-preview)
    ("r" "Refresh (export + preview)" emacs-mcp-presentation-refresh)
    ("b" "Open HTML in browser" emacs-mcp-presentation-open-current-html)
    ("x" "Export and preview" emacs-mcp-presentation-export-and-preview)]
   ["MCP"
    ("o" "Generate outline" emacs-mcp-presentation-generate-outline)
    ("m" "Save slide to memory" emacs-mcp-presentation-save-slide-to-memory)]])

;;;; MCP API Functions (for Claude to call programmatically):

(defun emacs-mcp-presentation-api-refresh (file)
  "API: Open FILE, export, and preview. Returns status message.
Use this from Claude to refresh a presentation after edits."
  (find-file file)
  (emacs-mcp-presentation-refresh)
  (format "Refreshed presentation: %s" file))

(defun emacs-mcp-presentation-api-export-html (file)
...
(defun emacs-mcp-presentation-api-export-pdf (file)
...
(defun emacs-mcp-presentation-api-open-browser (file)
...
(defun emacs-mcp-presentation-api-get-info (file)
...
(defun emacs-mcp-presentation--addon-init ()
  "Initialize the presentation addon."
  (require 'ox-beamer nil t)
  (require 'ox-reveal nil t)
  (message "emacs-mcp-presentation: initialized"))

(defun emacs-mcp-presentation--addon-shutdown ()
  "Shutdown the presentation addon."
  (message "emacs-mcp-presentation: shutdown"))
...
(with-eval-after-load 'emacs-mcp-addons
  (emacs-mcp-addon-register
   ...
   :requires '(emacs-mcp-api org)
   :provides '(emacs-mcp-presentation-transient)
   :init #'emacs-mcp-presentation--addon-init
   :shutdown #'emacs-mcp-presentation--addon-shutdown))

(provide 'emacs-mcp-presentation)
;;; emacs-mcp-presentation.el ends here
